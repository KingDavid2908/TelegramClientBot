...
import os  # For accessing operating system and locating files on your computing
...
#personal group id created for experimenting on my bot = -1002427302453. Replace where you see this number with the group id of your choice
...
async def receive_message(event):
    ...
    # Don't uncomment the next message or you would start spamming group messages and replying to all messages
...
async def group_message_handler(event):
    sender = await event.get_sender()
    if sender is None:  # checks if the sender is None to avoid throwing error if it is a bot or 
